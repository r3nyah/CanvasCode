...
class Sketch{
  ...
  Map<String,dynamic> toJson(){
    List<Map> pointsMap = points.map((e) => {
      'dx': e.dx,
      'dy': e.dy
    }).toList();
    return{
  ...
  factory Sketch.fromJson(Map<String,dynamic>json){
    List<Offset> points = (json['points']as List).map((e) => Offset(
      e['dx'],
      e['dy']
    )).toList();
    return Sketch(
...
extension SketchTypeExtension on String{
  toSketchTypeEnum(){
    return SketchType.values.firstWhere((element) => element.toString() == 'SketchType.${this}');
  }
}
